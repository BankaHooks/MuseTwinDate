...
async def pick_candidate_simple(session: AsyncSession, user: User) -> Tuple[Optional[User], float]:
    ...
    for candidate in pool:
        ...
        if user_interests and cand_interests:
            ...
            if common:
                score += len(common) / max(len(user_interests), 1) * 0.1
